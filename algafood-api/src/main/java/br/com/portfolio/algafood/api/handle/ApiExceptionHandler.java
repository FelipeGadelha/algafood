	...
	protected ResponseEntity<Object> handleExceptionInternal(
		...
		if (Objects.isNull(body)) {
			body = ExceptionDetails
				...
				.status(status.value())
				.title(HttpStatus.valueOf(status.value()).getReasonPhrase() + DOCUMENTATION)
				.details(ex.getMessage())
				.developerMessage(ex.getClass().getName())
				.build();
		} else if (body instanceof ExceptionStatus exStatus) {
			body = ExceptionDetails
				.builder()
				.timestamp(OffsetDateTime.now())
				.status(status.value())
...
//	}
	...
	private ResponseEntity<Object> argumentNotValidException(
		Exception exception,
	 	HttpHeaders headers,
		HttpStatusCode status,
		WebRequest request,
		...
		Map<String, Set<String>> map = fieldErros.stream()
			.collect(
	...
	private ExceptionDetails createExceptionDetailsBuilder(
		Exception ex,
		HttpStatusCode status,
